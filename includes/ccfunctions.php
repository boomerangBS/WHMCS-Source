...
function updateCCDetails($userid, $cardtype, $cardnum, $cardcvv, $cardexp, $cardstart, $cardissue, $noremotestore = "", $fullclear = "", $paymentGateway = "", &$payMethodRef = false, $ccDescription = "", $billingContactId = NULL, $invoiceId = NULL)
{
    ...
    if($cardtype) {
        ...
        } elseif((int) ("20" . substr($cardexp, 2) . substr($cardexp, 0, 2)) < (int) date("Ym")) {
    ...
    while ($data = mysql_fetch_array($result)) {
        ...
        if($gatewayInterface->functionExists("storeremote")) {
            ...
            $debugdata = sprintf("UserID => %s\n", $rparams["clientdetails"]["userid"]);
            if(isset($captureresult["rawdata"])) {
                if(is_array($captureresult["rawdata"])) {
                    $debugdata = array_merge(["UserID" => $rparams["clientdetails"]["userid"]], $captureresult["rawdata"]);
                } else {
                    $debugdata .= $captureresult["rawdata"];
                }
            }
            if($captureresult["status"] == "success") {
                ...
                if(array_key_exists("cardhint", $captureresult) && $captureresult["cardhint"]) {
                    $cardLastFour = $captureresult["cardhint"];
                }
                ...
                if(array_key_exists("description", $captureresult) && $captureresult["description"]) {
                    $ccDescription = $captureresult["description"];
                }
...
function captureCCPayment($invoiceid, $cccvv = "", $passedparams = false, WHMCS\Payment\PayMethod\Model $payMethod = NULL)
{
    ...
    } elseif(empty($params["cardnum"]) && empty($params["gatewayid"]) && empty($params["cccvv"]) && empty($params["bankacct"])) {
...
function saveNewRemoteCardDetails(array $remoteDetails, WHMCS\Module\Gateway $gateway, $clientId)
{
    ...
    if(isset($remoteDetails["cardhint"])) {
        $payment->setLastFour($remoteDetails["cardhint"]);
    }
    if($cardType) {
        $payment->setCardType($cardType);
    }
