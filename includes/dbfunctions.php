        ...
        if(is_array($where)) {
            ...
            foreach ($where as $key => $value) {
                if(is_array($value)) {
                    ...
                    if($value["sqltype"] == "LIKE") {
                    ...
                    } elseif($value["sqltype"] == "NEQ") {
                        $criteria[] = $key . "!='" . db_escape_string($value["value"]) . "'";
                    } elseif($value["sqltype"] == ">" && db_is_valid_amount($value["value"])) {
                        $criteria[] = $key . ">" . db_escape_string($value["value"]) . "";
                    } elseif($value["sqltype"] == "<" && db_is_valid_amount($value["value"])) {
                        $criteria[] = $key . "<" . db_escape_string($value["value"]) . "";
                    } elseif($value["sqltype"] == "<=" && db_is_valid_amount($value["value"])) {
                        ...
                        $criteria[] = $key . ">=" . db_escape_string($value["value"]) . "";
                    } elseif($value["sqltype"] == "TABLEJOIN") {
                        $criteria[] = $key . "=" . db_escape_string($value["value"]) . "";
                    } elseif($value["sqltype"] == "IN") {
...
function tokenizeOrderby($input, $default_ordering = "ASC")
{
    ...
    while ($i < 30 && $field !== false) {
        ...
        if(!$field) {
            ...
            } elseif(strpos($rev_field, $order_down_rev) === 0) {
...
function update_query($table, $array, $where, $resource = NULL)
{
    ...
    foreach ($array as $key => $value) {
        ...
        if(is_array($value)) {
            ...
            $query .= "NULL,";
        } elseif(substr($value, 0, 2) === "+=" && db_is_valid_amount(substr($value, 2))) {
    ...
    if(is_array($array) && 0 < count($array)) {
        ...
        $query_count++;
    } elseif($CONFIG["SQLErrorReporting"]) {
