...
function acceptjs_config_validate(array $params = [])
{
...
function acceptjs_capture(array $params = [])
{
    ...
    } elseif((!is_array($gatewayId) || json_last_error() !== JSON_ERROR_NONE) && $params["cardnum"]) {
        try {
            $merchantAuthentication = new net\authorize\api\contract\v1\MerchantAuthenticationType();
            $merchantAuthentication->setName($params["apiLoginId"]);
            $merchantAuthentication->setTransactionKey($params["transactionKey"]);
...
function acceptjs_refund(array $params = [])
{
    $gatewayId = json_decode(WHMCS\Input\Sanitize::decode($params["gatewayid"]), true);
    $apiUrl = net\authorize\api\constants\ANetEnvironment::PRODUCTION;
    if($params["testMode"]) {
...
function acceptjs_storeremote(array $params = [])
{
    ...
    } elseif($gatewayId && is_array($gatewayId) && $dataValue && $dataDescriptor) {
        try {
            $customerProfileId = $gatewayId["customer"];
            $merchantAuthentication = new net\authorize\api\contract\v1\MerchantAuthenticationType();
            $merchantAuthentication->setName($params["apiLoginId"]);
    ...
    } elseif($gatewayId && is_array($gatewayId) && $params["action"] === "update") {
        try {
            $customerProfileId = $gatewayId["customer"];
            $merchantAuthentication = new net\authorize\api\contract\v1\MerchantAuthenticationType();
            $merchantAuthentication->setName($params["apiLoginId"]);
    ...
    } elseif($gatewayId && is_array($gatewayId) && $params["action"] === "delete") {
        try {
            $customerProfileId = $gatewayId["customer"];
            $customerPaymentProfileId = $gatewayId["payment"];
            $merchantAuthentication = new net\authorize\api\contract\v1\MerchantAuthenticationType();
...
function acceptjs_post_checkout(array $params = [])
{
    try {
        $dataDescriptor = WHMCS\Session::getAndDelete("dataDescriptor");
        $dataValue = WHMCS\Session::getAndDelete("dataValue");
...
function acceptjs_cc_validation(array $params = [])
{
...
function acceptjs_credit_card_input(array $params = [])
{
