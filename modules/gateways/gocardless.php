...
function gocardless_config(array $params)
{
    ...
    if(array_key_exists("accessToken", $params) && $params["accessToken"]) {
        try {
            ...
            } elseif($creditor && $creditor->verification_status == "in_review") {
            ...
            } elseif($creditor && $creditor->verification_status == "successful") {
...
function gocardless_link($params)
{
    ...
    if(App::isInRequest("payment") && App::getFromRequest("payment") == 1) {
        ...
        $scheme = WHMCS\Module\Gateway\GoCardless\GoCardless::SCHEMES[$params["currency"]] ?? NULL;
        if(!$scheme) {
            $error = Lang::trans("invoicepaymentfailedconfirmation");
            return "<div class=\"alert alert-danger\">" . $error . "</div>";
        }
        $postParams["redirect_flows"] = ["session_token" => "SESSION_" . $params["clientdetails"]["id"] . "_" . $params["invoiceid"], "success_redirect_url" => $successUrl, "prefilled_customer" => $customer, "scheme" => $scheme];
        try {
...
function gocardless_capture(array $params)
{
    ...
    } elseif($paymentReference && in_array($paymentReference->remoteStatus, WHMCS\Module\Gateway\GoCardless\Resources\Payments::CANCELLED_STATES)) {
        $paymentReference = NULL;
    }
    if(!$paymentReference) {
        try {
...
function gocardless_mandate_import_start(WHMCS\Module\Gateway\GoCardless\Api\Client $client, $customerId) : WHMCS\Module\Gateway\GoCardless\Api\Client
{
