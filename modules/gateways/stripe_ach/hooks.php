...
add_hook("ClientAreaFooterOutput", 1, function (array $vars) {
    ...
    if(!(in_array("stripe", $activeGateways) || in_array("stripe_sepa", $activeGateways))) {
...
Hook::add("AddGlobalWarnings", 1, function () {
    $warningArray = WHMCS\Module\Gateway\Stripe\Admin\Warning::message("stripe_ach");
    if(!$warningArray) {
        return [];
    }
    return ["stripeAchWarning" => $warningArray];
});
