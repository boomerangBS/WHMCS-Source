...
function authorizecim_config_validate(array $params = [])
{
...
function authorizecim_capture($params)
{
    ...
    if(!empty($params["testmode"])) {
        $extraOptions["x_solution_id"] = "AAA100302";
    ...
    $data = curlCall($url, $xml, ["HEADER" => ["Content-Type: text/xml"]]);
    logModuleCall("authorizecim", "capture", $xml, $data);
    $xmldata = XMLtoArray($data);
    ...
    $response = $xmldata["CREATECUSTOMERPROFILETRANSACTIONRESPONSE"]["TRANSACTIONRESPONSE"] ?? NULL;
    return ["status" => "error", "rawdata" => $data, "declinereason" => $response["ERRORS"]["ERROR"]["ERRORTEXT"] ?? NULL];
}
