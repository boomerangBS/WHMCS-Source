...
class Standard
{
    ...
    public function callback(array $params = [])
    {
        ...
        if($params["secretword"]) {
            $hashInput = $_REQUEST["sale_id"] . $params["vendornumber"] . $_REQUEST["invoice_id"] . $params["secretword"];
            if(!Helper::isValidHash($hashInput, $_POST["md5_hash"] ?? "")) {
        ...
        } elseif($message_type == "ORDER_CREATED" && $params["skipfraudcheck"]) {
        ...
        } elseif($message_type == "RECURRING_INSTALLMENT_FAILED") {
    ...
    {
        if($params["secretword"]) {
            $hashInput = $params["secretword"] . $params["vendornumber"] . $_REQUEST["x_trans_id"] . $_REQUEST["x_amount"];
            if(!Helper::isValidHash($hashInput, $_REQUEST["x_MD5_Hash"] ?? "")) {
