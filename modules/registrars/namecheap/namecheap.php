...
function namecheap_GetNameservers($params)
{
    ...
    try {
        ...
        $values["ns1"] = $ns[0] ?? NULL;
        $values["ns2"] = $ns[1] ?? NULL;
        $values["ns3"] = $ns[2] ?? NULL;
        $values["ns4"] = $ns[3] ?? NULL;
        $values["ns5"] = $ns[4] ?? NULL;
    } catch (Exception $e) {
...
function namecheap_SaveNameservers($params)
{
    ...
    foreach ($nameservers as $k => $v) {
        if(!$v) {
            unset($nameservers[$k]);
        } elseif(!in_array($v, $defaultNsServers)) {
            $defaultNs = false;
        }
    }
    try {
...
function namecheap_GetRegistrarLock($params)
{
    ...
    try {
        ...
        $result = $api->parseResponse($response);
        $remoteLockStatus = $result["DomainGetRegistrarLockResult"]["@attributes"]["RegistrarLockStatus"] ?? "";
        $lockstatus = "true" == $remoteLockStatus;
...
function namecheap_RegisterDomain($params)
{
    ...
    try {
        ...
        if("eu" == strtolower($tld)) {
            ...
            } elseif(!empty($params["additionalfields"]["Job Title"])) {
...
function namecheap_TransferSync($params)
{
    ...
    try {
        ...
        $result = $api->parseResponse($response);
        $domains = $api->parseResultSyncHelper($result["TransferGetListResult"]["Transfer"] ?? []);
    } catch (Exception $e) {
        $values["error"] = $e->getMessage();
        return $values;
    }
