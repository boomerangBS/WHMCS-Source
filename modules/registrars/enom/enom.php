...
function enom_config_validate(array $params)
{
    ...
    if(0 < $eNom->Values["ErrCount"]) {
        ...
        if(substr($error, 0, 39) == "User not permitted from this IP address") {
            ...
            $href = "https://go.whmcs.com/1885/enom#activation";
...
function enom_CheckAvailability($params)
{
    ...
    foreach (array_chunk($domains, $maxDomainsInRequest) as $domainChunk) {
        ...
        for ($i = 1; $i <= $Enom->Values["DomainCount"]; $i++) {
            ...
            } elseif(!$params["premiumEnabled"] && $isPremium) {
        ...
        foreach (array_chunk($domainsToCheck, $maxDomainsInRequest) as $domainChunk) {
            ...
            if($params["premiumEnabled"]) {
                for ($i = 1; $i <= $EnomPremium->Values["DomainCount"]; $i++) {
                    if(isset($EnomPremium->Values["IsPremiumName" . $i]) && $EnomPremium->Values["IsPremiumName" . $i] == "true") {
    ...
    for ($i = 1; $i <= $numberOfSuggestions; $i++) {
        if(!isset($Enom->Values["sld" . $i])) {
            ...
            if($params["isIdnDomain"] && $idn == "false") {
                ...
                } elseif(!$params["premiumEnabled"] && $premium) {
                    $domainSearchResult->setStatus($domainSearchResult::STATUS_RESERVED);
                }
                $results[] = $domainSearchResult;
            }
...
function enom_GetNameservers($params)
{
    ...
    $Enom->AddParam("uid", $params["Username"]);
    $Enom->AddParam("pw", $params["Password"]);
    $Enom->AddParam("tld", enom__getAsciiValue($params, "tld"));
    $Enom->AddParam("sld", enom__getAsciiValue($params, "sld"));
...
{
    $params = injectDomainObjectIfNecessary($params);
    $nameservers = enom__prepareNameserversForSave($params);
    $Enom = new WHMCS\Module\Registrar\Enom\ApiInterface();
    $Enom->AddParam("uid", $params["Username"]);
    $Enom->AddParam("pw", $params["Password"]);
    $Enom->AddParam("tld", enom__getAsciiValue($params, "tld"));
    $Enom->AddParam("sld", enom__getAsciiValue($params, "sld"));
    for ($i = 1; $i <= 5; $i++) {
        $Enom->AddParam("NS" . $i, enom__getAsciiValue($nameservers, "ns" . $i));
    }
    $Enom->AddParam("command", "modifyns");
    $Enom->DoTransaction($params);
    $values["error"] = $Enom->Values["Err1"] ?? "";
    for ($i = 1; $i <= 5; $i++) {
        $values["error"] = enom__replaceAsciiValue($params, $values["error"], "ns" . $i);
    }
    return $values;
}
...
function enom_RegisterDomain($params)
{
    ...
    $Enom->AddParam("tld", enom__getAsciiValue($params, "tld"));
    $Enom->AddParam("sld", enom__getAsciiValue($params, "sld"));
    if(enom__isIDN($params)) {
        $Enom->AddParam("IDNCode", $params["idnLanguage"]);
    }
    ...
    if($params["DefaultNameservers"]) {
        ...
        $Enom->AddParam("NS1", enom__getAsciiValue($params, "ns1"));
        $Enom->AddParam("NS2", enom__getAsciiValue($params, "ns2"));
        $Enom->AddParam("NS3", enom__getAsciiValue($params, "ns3"));
        $Enom->AddParam("NS4", enom__getAsciiValue($params, "ns4"));
        $Enom->AddParam("NS5", enom__getAsciiValue($params, "ns5"));
    }
    ...
    $Enom->DoTransaction($params);
    $values["error"] = NULL;
    if(!empty($Enom->Values["Err1"])) {
        $values["error"] = enom__replaceAsciiValue($params, $Enom->Values["Err1"], "domain");
    }
    $rrpCode = (int) ($Enom->Values["RRPCode"] ?? 0);
...
function enom_TransferDomain(array $params)
{
    ...
    $Enom->AddParam("TLD1", enom__getAsciiValue($params, "tld"));
    $Enom->AddParam("SLD1", enom__getAsciiValue($params, "sld"));
...
function enom_RenewDomain($params)
{
    ...
    } elseif(stripos($RegistrationStatus, "Expired") !== false) {
        $Enom->AddParam("DomainName", sprintf("%s.%s", enom__getAsciiValue($params, "sld"), enom__getAsciiValue($params, "tld")));
        $Enom->AddParam("NumYears", $params["regperiod"]);
    ...
    if(0 < $Enom->Values["ErrCount"]) {
        $values["error"] = $Enom->Values["Err1"] ?? "";
        $values["error"] = enom__replaceAsciiValue($params, $values["error"], "domain");
    } elseif($params["idprotection"]) {
        $Enom->NewRequest();
        $Enom->AddParam("uid", $params["Username"]);
        $Enom->AddParam("pw", $params["Password"]);
...
function enom_SaveContactDetails($params)
{
    ...
    for ($i = 0; $i <= 2; $i++) {
        ...
        $Enom->AddParam(sprintf("%sFax", $contacttypes[$i]), $params["contactdetails"][$contacttypes[$i]]["Fax"] ?? "");
...
function enom_RegisterNameserver($params)
{
    ...
    $Enom->AddParam("NSName", enom__getAsciiValue($params, "nameserver"));
    $Enom->AddParam("IP", $params["ipaddress"]);
    $Enom->DoTransaction($params);
    if(!empty($Enom->Values["Err1"])) {
        $error = $Enom->Values["Err1"];
    }
    if(!empty($Enom->Values["ResponseString1"])) {
...
function enom_ModifyNameserver($params)
{
    ...
    $Enom->AddParam("pw", $params["Password"]);
    $Enom->AddParam("Add", "true");
    $Enom->AddParam("NS", enom__getAsciiValue($params, "nameserver"));
...
function enom_TransferSync($params)
{
    ...
    if($Enom->Values["Err1"] || $Enom->Values["ResponseString1"]) {
        ...
        } elseif(in_array($statusid, $cancelledstatusids)) {
            $values["failed"] = true;
            $values["reason"] = $statusdesc;
        } elseif(in_array($statusid, $pendingstatusids)) {
...
function enom_GetPremiumPrice(array $params)
{
    ...
    $Enom->AddParam("pw", $params["Password"]);
    $Enom->AddParam("SLD", enom__getAsciiValue($params, "sld"));
    $Enom->AddParam("TLD", ltrim(enom__getAsciiValue($params, "tld"), "."));
    $Enom->AddParam("command", "check");
    $Enom->AddParam("IncludePrice", true);
    $Enom->DoTransaction($params);
    if(0 < $Enom->Values["ErrCount"]) {
...
function enom_IDProtectToggle(array $params)
{
    ...
    $Enom->DoTransaction($params);
    $idProtectionAllowed = $Enom->Values["WPPSAllowed"] ?? false;
    $idProtectionSubscription = $Enom->Values["WPPSExists"] ?? false;
    $idProtectionEnabled = $Enom->Values["WPPSEnabled"] ?? false;
    ...
    foreach ($arrayData["services"]["entry"] as $service) {
        ...
        if($service["attr"]["name"] === "dnsserver" && $service["configuration"]["dns"]) {
            ...
            foreach ($service["configuration"]["dns"] as $nameserver) {
                ...
                $nameservers["ns" . $nsCounter] = WHMCS\Domains\Idna::fromPunycode($ns);
                $nsCounter++;
            }
        }
        if(!$irtpDisabled) {
            if($service["attr"]["name"] === "irtpsettings") {
                $irtpOptOutStatus = false;
                if(!empty($service["irtpsetting"]["optout"]["value"])) {
                    $irtpOptOutStatus = $service["irtpsetting"]["optout"]["value"] === "True";
                }
                $irtpTransferLock = false;
                if(!empty($service["irtpsetting"]["transferlock"]["value"])) {
                    $irtpTransferLock = $service["irtpsetting"]["transferlock"]["value"] === "True";
                }
                $icannTld = false;
                if(!empty($service["irtpsetting"]["icanncompliant"]["value"])) {
...
function enom_ResendIRTPVerificationEmail(array $params)
{
    ...
    $eNom->AddParam("uid", $params["Username"]);
    $eNom->AddParam("pw", $params["Password"]);
    $eNom->AddParam("DomainName", enom__getAsciiValue($params, "domainname"));
...
function enom_GetTldPricing(array $params)
{
    ...
    foreach ($tldPricing as $tldPrice) {
        ...
        } elseif($item->getExtension() == ".com.mx") {
            $item->setMaxYears(5);
        }
        $results[] = $item;
    }
...
function enom_set_initial_contact_data(WHMCS\Module\Registrar\Enom\ApiInterface $Enom, array $params) : WHMCS\Module\Registrar\Enom\ApiInterface
{
    $jobTitle = "";
    if($params["companyname"]) {
        $jobTitle = "Director";
...
function enom_populate_additional_fields(WHMCS\Module\Registrar\Enom\ApiInterface $Enom, array $params) : WHMCS\Module\Registrar\Enom\ApiInterface
{
    if($params["domainObj"]->getLastTLDSegment() == "us") {
    ...
    } elseif($params["domainObj"]->getLastTLDSegment() == "fm") {
        $Enom->AddParam("fm_agreedelete", "YES");
    } elseif($params["domainObj"]->getLastTLDSegment() == "be") {
        $Enom->AddParam("be_agreedelete", "YES");
    } elseif($params["domainObj"]->getLastTLDSegment() == "nz") {
    ...
    } elseif($params["domainObj"]->getLastTLDSegment() == "sg") {
        ...
        if($additional["Legal Type"] === "Individual") {
            $Enom->AddParam("fr_registrantlegalid", $additional["Legal ID"]);
        }
    } elseif($params["domainObj"]->getLastTLDSegment() == "nu") {
    ...
    } elseif($params["domainObj"]->getLastTLDSegment() == "dev") {
        $devAgreement = "";
        if($params["additionalfields"][".DEV SSL Agreement"]) {
            $devAgreement = "I Agree";
        }
        $Enom->AddParam("dev_agree", $devAgreement);
    } elseif($params["domainObj"]->getLastTLDSegment() == "app") {
        $appAgreement = "";
        if($params["additionalfields"][".APP SSL Agreement"]) {
            $appAgreement = "I Agree";
        }
        $Enom->AddParam("app_agree", $appAgreement);
    } elseif($params["domainObj"]->getLastTLDSegment() == "bio") {
        $bioAgreement = "";
        if($params["additionalfields"][".BIO SSL Agreement"]) {
            $bioAgreement = "I Agree";
        }
        $Enom->AddParam("bio_agree", $bioAgreement);
    } elseif($params["domainObj"]->getLastTLDSegment() == "nyc") {
        $nexusValue = $params["additionalfields"]["nexusCategory"] == "NEXUS_CATEGORY_1" ? "ORG" : "INDIV";
        $Enom->AddParam("nyc_nexuscategory", $nexusValue);
    }
}
function enom_getExtAttributes($params, string $tld)
{
    $eNom = new WHMCS\Module\Registrar\Enom\ApiInterface();
    $eNom->NewRequest();
    $eNom->AddParam("uid", $params["Username"]);
...
function enom__isIDN($params)
{
    return $params["is_idn"] && !empty($params["idnLanguage"]);
}
function enom__getAsciiValue($params, string $uniCodeValue)
{
    $punyCodeValue = $uniCodeValue . "_punycode";
    if(!isset($params[$punyCodeValue])) {
        return $params[$uniCodeValue];
    }
    if($params[$uniCodeValue] != $params[$punyCodeValue]) {
        return $params[$punyCodeValue];
    }
    return $params[$uniCodeValue];
}
function enom__replaceAsciiValue($params, string $dataString, string $uniCodeKey)
{
    $asciiValue = enom__getasciivalue($params, $uniCodeKey);
    return str_replace($asciiValue, $params[$uniCodeKey], $dataString);
}
function enom__prepareNameserversForSave($params)
{
    $nsValues = [];
    $nsCount = 0;
    for ($i = 1; $i <= 5; $i++) {
        $nsKey = "ns" . $i;
        if(!in_array($params[$nsKey], ["dns1.name-services.com", "dns2.name-services.com", "dns3.name-services.com", "dns4.name-services.com", "dns5.name-services.com"])) {
            $nsCount++;
            $nsValues["ns" . $nsCount] = $params[$nsKey];
            $nsValues["ns" . $nsCount . "_punycode"] = $params[$nsKey . "_punycode"];
            unset($params[$nsKey]);
        }
    }
    for ($i = 1; $i <= 5; $i++) {
        $nsKey = "ns" . $i;
        if(isset($params[$nsKey])) {
            $nsCount++;
            $nsValues["ns" . $nsCount] = $params[$nsKey];
            $nsValues["ns" . $nsCount . "_punycode"] = $params[$nsKey . "_punycode"];
        }
