...
function registercom_RegisterDomain($params)
{
    ...
    if($data["serviceResponse"]["status"]["statusCode"]["value"] != "1000" && $data["serviceResponse"]["status"]["statusCode"]["value"] != "1005") {
        $values = ["error" => sprintf("Could not add or update user account %s %s with Registrar for the domainAdd request of %s", $AdminFirstName, $AdminLastName, $domain)];
        return $values;
    }
...
function registercom_TransferDomain($params)
{
    ...
    $data = registercom_curlCall($xml_adduser, $params);
    if($data["serviceResponse"]["status"]["statusCode"]["value"] != "1000" && $data["serviceResponse"]["status"]["statusCode"]["value"] != "1005") {
        $values = ["error" => sprintf("Could not add or update user account %s %s with Registrar for the domainTransferIn request of %s", $AdminFirstName, $AdminLastName, $domain)];
        return $values;
    }
...
function registercom_GetContactDetails($params)
{
    ...
    return ["Registrant" => ["First Name" => $data[1]["firstName"]["value"] ?? "", "Last Name" => $data[1]["lastName"]["value"] ?? ""], "Admin" => ["First Name" => $data[0]["firstName"]["value"] ?? "", "Last Name" => $data[0]["lastName"]["value"] ?? ""]];
}
    ...
    foreach ($xml_values as $data) {
        ...
        if($type == "open") {
            ...
            if(!isset($current[$tag])) {
                $current[$tag] = $result;
            } elseif(is_array($current[$tag]) && $get_attributes == 0 || isset($current[$tag][0]) && is_array($current[$tag][0]) && $get_attributes == 1) {
                array_push($current[$tag], $result);
            } else {
                $current[$tag] = [$current[$tag], $result];
            }
