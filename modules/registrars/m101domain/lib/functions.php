...
function m101domain_SaveNameservers($params)
{
    try {
        ...
        $to_add = array_values(array_diff($new_nameservers, $domain->ns));
...
function m101domain_Sync($params)
{
    ...
    try {
        ...
        } elseif(in_array("pendingDelete", $info->status) || in_array("redemptionPeriod", $info->status)) {
            $values["active"] = false;
            $values["expired"] = true;
        }
        return $values;
...
function m101domain_command($params, $xml, $command = "")
{
    ...
    while ($node) {
        ...
        } elseif($node->localName == "value" && $node->namespaceURI == $root->namespaceURI && ($text = m101domain_FindNode($node, "text", $root->namespaceURI))) {
            $text->normalize();
    ...
    if($inf_data = m101domain_findnode($ret->resData, "resData", "urn:ietf:params:xml:ns:host-1.0")) {
        ...
        while ($node) {
            if($node instanceof DOMElement && $node->namespaceURI == "urn:ietf:params:xml:ns:host-1.0") {
                ...
                } elseif($node->localName == "crDate") {
                    $node->normalize();
                    $host->cr_date = strtotime($node->firstChild->nodeValue);
                } elseif($node->localName == "crID") {
                    $node->normalize();
