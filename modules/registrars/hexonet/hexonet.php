        ...
        if($domainStatusCode == "210") {
            ...
            if($domainStatusCode == "211") {
                ...
                if(stripos($domainReason, "Reserved") !== false) {
                    $isReserved = true;
                    $isPremium = false;
                } elseif(stripos($domainReason, "Premium") !== false) {
                    $isPremium = true;
                    $isReserved = false;
                }
...
function hexonet_SaveContactDetails($params)
{
    ...
    foreach ($map as $ctype => $ptype) {
        if(isset($unstrippedparams["contactdetails"][$ptype]["First Name"])) {
        ...
        $command[$ctype] = ["FIRSTNAME" => $decode($p["First Name"] ?? ""), "LASTNAME" => $decode($p["Last Name"] ?? ""), "ORGANIZATION" => $decode($p["Company Name"] ?? ""), "STREET" => $decode($p["Address"] ?? ""), "CITY" => $decode($p["City"] ?? ""), "STATE" => $decode($p["State"] ?? ""), "ZIP" => $decode($p["Postcode"] ?? ""), "COUNTRY" => $decode($p["Country"] ?? ""), "PHONE" => $decode($p["Phone"] ?? ""), "FAX" => $decode($p["Fax"] ?? ""), "EMAIL" => $decode($p["Email"] ?? "")];
        if(0 < strlen($p["Address 2"])) {
            $command[$ctype]["STREET"] .= sprintf(" , %s", $decode($p["Address 2"] ?? ""));
        }
        unset($decode);
    }
    if($tldSegment == "it") {
        ...
        } elseif($newRegistrantDetails["Company Name"] != $currentDetails["Company Name"]) {
            $commandOverride = ["COMMAND" => "TradeDomain", "DOMAIN" => $domain->getDomain()];
        } elseif($newRegistrantDetails["Country"] != $currentDetails["Country"]) {
            $commandOverride = ["COMMAND" => "TradeDomain", "DOMAIN" => $domain->getDomain()];
        } else {
...
function hexonet_RegisterDomain($params)
{
    ...
    if($tldSegment == "it") {
    ...
    } elseif(in_array($tldSegment, ["fr", "re", "pm", "tf", "wf", "yt"])) {
        ...
        } elseif(!empty($params["additionalfields"]["SIRET Number"])) {
        ...
        } elseif(!empty($params["additionalfields"]["Individuals Document Number"])) {
        ...
        } elseif(!empty($params["additionalfields"]["Individuals Issuing Country"])) {
...
function hexonet_Sync($params)
{
    ...
    if($params["tld"] == "swiss") {
        ...
        } elseif(in_array($response["CODE"], [531, 545])) {
...
function hexonet_get_contact_info($contact, &$params)
{
    ...
    if(true) {
        ...
        if(isset($response["PROPERTY"]["STREET"]) && count($response["PROPERTY"]["STREET"]) < 2 && preg_match("/^(.*) , (.*)/", $response["PROPERTY"]["STREET"][0], $m)) {
        ...
        if(preg_match("/[.]ca\$/i", $domain) && isset($response["PROPERTY"]["X-CA-LEGALTYPE"])) {
            ...
            } elseif(!isset($response["PROPERTY"]["ORGANIZATION"]) || !$response["PROPERTY"]["ORGANIZATION"][0]) {
...
function hexonet_config($params)
{
    ...
    if(0 < strlen($params["ProxyServer"])) {
