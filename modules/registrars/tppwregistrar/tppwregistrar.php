...
abstract class TPPW_API
{
    ...
    protected function __construct($script, $params)
    {
        ...
        if(function_exists("logModuleCall")) {
            logModuleCall(TPPW_APIUtils::$MODULE_NAME, $this->url, $this->params, "", "", [$params["Password"], $params["Login"]]);
        }
        if($params["Debug"]) {
            foreach ([$params["Password"], $params["Login"]] as $sensitive) {
                $message = str_replace($sensitive, str_repeat("*", strlen($sensitive)), $message);
            }
            logactivity($message);
        }
    }
    ...
    protected function execute($postParams, $existingSession = true)
    {
        ...
        if(function_exists("logModuleCall")) {
            logModuleCall(TPPW_APIUtils::$MODULE_NAME, $this->url, $postFields, $response, "", [$this->params["Password"], $this->params["Login"]]);
        }
        if($this->params["Debug"]) {
            foreach ([$this->params["Password"], $this->params["Login"]] as $sensitive) {
                $message = str_replace($sensitive, str_repeat("*", strlen($sensitive)), $message);
            }
            logactivity($message);
        }
