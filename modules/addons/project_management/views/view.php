...
if($project->permissions()->check("Associate Tickets")) {
    ...
    echo $language["associateTicket"];
    echo "                </a>\n                ";
    if($project->permissions()->check("Open New Ticket")) {
        echo "                    <button class=\"btn btn-success\" data-toggle=\"modal\" data-target=\"#modalOpenTicket\">\n                        <i class=\"fas fa-plus\"></i>\n                        ";
...
if($project->permissions()->check("Associate Invoice")) {
    echo "                <button class=\"btn btn-default\" data-toggle=\"modal\" data-target=\"#modalAssociateInvoice\">\n                    <i class=\"fas fa-sync\"></i>\n                    ";
    echo $language["associateInvoice"];
    echo "                </button>\n                ";
    if($project->permissions()->check("Create Invoice")) {
        echo "                    <button id=\"createInvoice\" class=\"btn btn-success";
        echo !$project->userid ? " disabled\" disabled=\"disabled" : "";
...
foreach ($output["files"] as $key => $file) {
    ...
    } elseif((int) $createdAt->diffInDays(WHMCS\Carbon::now()) === 1) {
...
echo $language["client"];
echo ":</label>\n                                <input type=\"text\" class=\"form-control\" id=\"newTicketClientRO\" readonly=\"readonly\"\n                                    value=\"";
echo isset($output["client"]) ? $output["client"]->fullName : "";
echo "\" />\n                            </div>\n                            ";
if($output["contacts"]) {
...
echo "                                </select>\n                            </div>\n                        </div>\n                        ";
if($project->permissions()->check("List Clients")) {
    echo "                            <div class=\"row padding-top-10\">\n                                <div class=\"col-md-3\">";
    echo $language["client"];
