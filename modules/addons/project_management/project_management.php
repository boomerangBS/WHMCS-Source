...
if(isset($_REQUEST["action"]) && $_REQUEST["action"] == "dl") {
    ...
    try {
        ...
        if(WHMCS\Admin::getID()) {
            $extraMessage = "This could indicate that the file is missing or that <a href=\"" . routePath("admin-setup-storage-index") . "\" target=\"_blank\">storage configuration settings" . "</a> are misconfigured. " . "<a href=\"https://go.whmcs.com/2009/file-not-found\" target=\"_blank\">" . "Learn more</a>";
        } else {
...
function project_management_upgrade(array $vars = [])
{
    ...
    if(version_compare($version, "2.3.2") === -1) {
        ...
        $permissions[15] = ["1" => "1", "2" => "1"];
        WHMCS\Database\Capsule::table("tbladdonmodules")->where("module", "project_management")->where("setting", "perms")->update(["value" => safe_serialize($permissions)]);
    }
    project_management_email_templates();
}
function project_management_output($vars)
{
    ...
    $project = NULL;
    $container = WHMCS\Module\Addon\ProjectManagement\Container::getInstance();
    $projectId = (int) App::getFromRequest("projectid");
    if($projectId) {
        $project = $container->make("WHMCS\\Module\\Addon\\ProjectManagement\\Project", ["projectId" => $projectId, "language" => $vars["_lang"]]);
    }
    if(!empty($_REQUEST["createproj"])) {
    ...
    $permisson = $container->make("WHMCS\\Module\\Addon\\ProjectManagement\\ComplexPermission");
    if($ajax || $ajaxModal) {
        check_token("WHMCS.admin.default");
        $action = App::getFromRequest("action");
        $router = new WHMCS\Module\Addon\ProjectManagement\Router();
        $router->setPermission($permisson);
        try {
    ...
    if(project_management_checkperm("Create New Projects")) {
        ...
        $headeroutput .= "</select>\n                </div>";
        if($permisson->check("List Clients")) {
...
{
    $permissions = WHMCS\Module\Addon\ProjectManagement\Container::getInstance()->make("WHMCS\\Module\\Addon\\ProjectManagement\\ComplexPermission");
