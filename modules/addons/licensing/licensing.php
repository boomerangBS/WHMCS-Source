...
{
    if(empty($vars["clientverifytool"])) {
        return false;
    }
    ...
    if($domain) {
        $check = true;
        foreach (WHMCS\Database\Capsule::table("mod_licensing")->where("validdomain", "LIKE", "%" . $domain . "%")->orWhere("validip", "LIKE", "%" . $domain . "%")->get() as $data) {
            if($data->addon_id != 0) {
            ...
            if(in_array($domain, $validDomains) || in_array($domain, $validIps)) {
                $results[] = ["productname" => $productName, "domain" => $validDomains[0], "ip" => $validIps[0], "status" => $data->status];
            }
        }
    }
