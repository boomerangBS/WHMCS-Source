...
function resellerclubssl_CreateAccount(array $params)
{
    ...
    if(isset($params["configoptions"]["Certificate Type"]) && is_int($params["configoptions"]["Certificate Type"])) {
...
function resellerclubssl_SSLStepOne($params)
{
    ...
    if($params["remoteid"] && !strpos($params["certtype"], "|")) {
        ...
        if(($certificateData["response"]["status"] ?? "") == "ERROR") {
            if(empty($certificateData["response"]["message"])) {
    ...
    switch ($method) {
        ...
        default:
            ...
            if(curl_errno($ch)) {
                ...
                $result["response"]["status"] = "ERROR";
                $result["response"]["message"] = "CURL Error: " . curl_errno($ch) . " - " . curl_error($ch) . " (IP: " . $ip . " & " . $ip2 . ")";
            } elseif($noDecode && is_numeric($data)) {
                $result = $data;
            } else {
            ...
            if(isset($result["response"]["message"]) && $result["response"]["message"] == "An unexpected error has occurred") {
                $result["response"]["message"] = "Login Failure or Unexpected Error";
            }
            return $result;
    }
}
function resellerclubssl_CustomActions($params) : WHMCS\Module\Server\CustomActionCollection
{
