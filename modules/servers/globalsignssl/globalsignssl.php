...
function globalsignssl_SSLStepTwo($params)
{
    ...
    } elseif($prodcode == "DomainSSL") {
        $prodcode = "DV_SHA2";
    } elseif($prodcode == "OrganizationSSL") {
        $prodcode = "OV_SHA2";
    } elseif($prodcode == "ExtendedSSL") {
        $prodcode = "EV_SHA2";
    }
    ...
    $orderkind = "new";
    if(isset($params["customfields"]["OrderKind"]) && $params["customfields"]["OrderKind"] == "transfer") {
        $orderkind = "transfer";
    }
    if(!empty($params["configoptions"]["ValidityPeriod"])) {
        $validityperiod = $params["configoptions"]["ValidityPeriod"];
    }
    if(!empty($params["configoptions"]["Years"])) {
...
function globalsignssl_SSLStepThree($params)
{
    ...
    } elseif($prodcode == "OV_SHA2") {
        $request["Request"]["SubID"] = "";
    ...
    if(0 <= $errorcode) {
        $orderid = $result->Response->OrderID;
    } elseif($result->Response->OrderResponseHeader->Errors->Error->ErrorCode) {
    ...
    } elseif($result->Response->OrderResponseHeader->Errors->Error[0]->ErrorCode) {
...
function globalsignssl_CustomActions($params) : WHMCS\Module\Server\CustomActionCollection
{
