...
if($action == "savedomain" && $domain) {
    ...
    if(function_exists($registrar . "_AdminDomainsTabFieldsSave")) {
        ...
        $params = ["domainid" => $id, "sld" => $domainObj->getUnicodeSecondLevel(), "tld" => $domainObj->getTopLevel(), "regperiod" => $regperiod, "registrar" => $registrar];
        unset($domainObj);
...
switch ($conf) {
    ...
    default:
        ...
        if($domainregistraractions) {
            $domainObj = new WHMCS\Domains\Domain($domain);
            $params = ["domainid" => $id, "sld" => $domainObj->getUnicodeSecondLevel(), "sld_punycode" => $domainObj->getPunycodeSecondLevel(), "tld" => $domainObj->getTopLevel(), "tld_punycode" => $domainObj->getPunycodeTopLevel(), "regperiod" => $registrationperiod, "registrar" => $registrar, "regtype" => $regtype];
            unset($domainObj);
            $adminbuttonarray = "";
            loadRegistrarModule($registrar);
            ...
            if($newlockstatus != $oldlockstatus) {
                ...
                if(!is_bool($values) && !empty($values["error"])) {
            ...
            if($regaction == "eppcode") {
                ...
                } elseif(!empty($values["eppcode"])) {
                    infoBox(AdminLang::trans("domains.epprequest"), sprintf("%s %s", Lang::trans("domaingeteppcodeis"), $values["eppcode"]), "success");
                } else {
            ...
            if($regaction == "custom") {
                ...
                if(!empty($values["error"])) {
                    infoBox(AdminLang::trans("domains.registrarerror"), $values["error"], "error");
                } else {
                    if(!$values["message"]) {
        ...
        while ($data = mysql_fetch_array($result)) {
            ...
            } elseif(in_array($domainliststatus, ["Expired", "Cancelled", "Fraud", "Transferred Away"])) {
        ...
        if($domainregistraractions) {
            if($domains->hasFunction("GetNameservers") || $domains->hasFunction("GetDomainInformation")) {
                $nsPunycodeValues = [];
                foreach ($nsvalues as $nsKey => $nsValue) {
                    $nsPunycodeValues[$nsKey] = NULL;
                    if(!is_null($nsValue)) {
                        try {
                            $nsPunycodeValues[$nsKey] = WHMCS\Domains\Idna::toPunycode($nsvalues[$nsKey]);
                        } catch (WHMCS\Exception\InvalidDomain $e) {
                            $nsPunycodeValues[$nsKey] = $e->getMessage();
                        }
                    }
                }
                echo "<tr>\n    <td class=\"fieldlabel\">";
                echo $aInt->lang("domains", "nameserver");
                echo " 1</td>\n    <td class=\"fieldarea nameserver-inline\" colspan=\"3\">\n        <input type=\"text\" name=\"ns1\" value=\"";
                echo $nsvalues["ns1"];
                echo "\" class=\"form-control input-300\" />\n        <input type=\"hidden\" name=\"oldns1\" value=\"";
                echo $nsvalues["ns1"];
                echo "\" />\n        ";
                if($nsPunycodeValues["ns1"] && $nsvalues["ns1"] !== $nsPunycodeValues["ns1"]) {
                    echo "<input type=\"text\" value=\"" . $nsPunycodeValues["ns1"] . "\"" . " class=\"form-control input-300 domain-input disabled\" readonly>";
                }
                echo "    </td>\n</tr>\n<tr>\n    <td class=\"fieldlabel\">";
                echo $aInt->lang("domains", "nameserver");
                ...
                echo $nsvalues["ns2"];
                echo "\" />\n        ";
                if($nsPunycodeValues["ns2"] && $nsvalues["ns2"] !== $nsPunycodeValues["ns2"]) {
                    echo "<input type=\"text\" value=\"" . $nsPunycodeValues["ns2"] . "\"" . " class=\"form-control input-300 domain-input disabled\" readonly>";
                }
                echo "    </td>\n</tr>\n<tr>\n    <td class=\"fieldlabel\">";
                echo $aInt->lang("domains", "nameserver");
                ...
                echo $nsvalues["ns3"];
                echo "\" />\n        ";
                if($nsPunycodeValues["ns3"] && $nsvalues["ns3"] !== $nsPunycodeValues["ns3"]) {
                    echo "<input type=\"text\" value=\"" . $nsPunycodeValues["ns3"] . "\"" . " class=\"form-control input-300 domain-input disabled\" readonly>";
                }
                echo "    </td>\n</tr>\n<tr>\n    <td class=\"fieldlabel\">";
                echo $aInt->lang("domains", "nameserver");
                ...
                echo $nsvalues["ns4"];
                echo "\" />\n        ";
                if($nsPunycodeValues["ns4"] && $nsvalues["ns4"] !== $nsPunycodeValues["ns4"]) {
                    echo "<input type=\"text\" value=\"" . $nsPunycodeValues["ns4"] . "\"" . " class=\"form-control input-300 domain-input disabled\" readonly>";
                }
                echo "    </td>\n</tr>\n<tr>\n    <td class=\"fieldlabel\">";
                echo $aInt->lang("domains", "nameserver");
                ...
                echo $nsvalues["ns5"];
                echo "\" />\n        ";
                if($nsPunycodeValues["ns5"] && $nsvalues["ns5"] !== $nsPunycodeValues["ns5"]) {
                    echo "<input type=\"text\" value=\"" . $nsPunycodeValues["ns5"] . "\"" . " class=\"form-control input-300 domain-input disabled\" readonly>";
                }
        ...
        foreach ($mailTemplates as $template) {
            $emailArray[$template->id] = $template->custom ? ["color" => "#efefef", "text" => $template->name] : $template->name;
        }
