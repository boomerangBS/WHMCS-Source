...
$deptid = App::getFromRequest("deptid");
$preventClientClosure = (bool) App::getFromRequest("preventClientClosure");
$selectedtickets = App::getFromRequest("selectedtickets");
$sub = $whmcs->get_req_var("sub");
$relatedService = "";
$relatedServiceType = "";
if(in_array($action, ["viewticket", "view", "gettags", "addTag", "removeTag", "split", "getmsg", "getticketlog", "getticketactions", "getclientlog", "gettickets", "updatereply", "makingreply", "endreply", "checkstatus", "changestatus", "changeflag", "loadpredefinedreplies", "getpredefinedreply", "getquotedtext", "getcontacts", "markdown", "parseMarkdown"])) {
...
if($action == "split") {
    ...
    try {
        ...
        parent::whereIn("id", $nids)->where("ticketid", "=", $id)->update(["ticketid" => $newTicketID]);
...
if($action == "getticketlog") {
    ...
    $ticketLog = WHMCS\Database\Capsule::table("tblticketlog")->where("tid", $id)->orderBy("date", "DESC")->orderBy("id", "DESC")->offset($offset)->limit($qlimit)->get();
    foreach ($ticketLog as $data) {
        $tabledata[] = [fromMySQLDate($data->date, true), "<div style=\"text-align:left;\">" . $data->action . "</div>"];
    }
    ...
    if(in_array($action, ["view", "viewticket"]) && validateAdminTicketAccess($id) === false && $aInt->getAdminUser()->hasPermission("Create Scheduled Ticket Actions")) {
        $aInt->assign("scheduledActionsAndActionsPanel", (new WHMCS\Support\Ticket\ScheduledActions\View\ScheduledActions())->factoryReplyView(WHMCS\Support\Ticket::find($id))->render());
    }
    if($action == "getticketactions") {
        check_token("WHMCS.admin.default");
        if(validateAdminTicketAccess($id) || !$aInt->getAdminUser()->hasPermission("View Scheduled Ticket Actions")) {
            WHMCS\Terminus::getInstance()->doExit();
        }
        echo (new WHMCS\Support\Ticket\ScheduledActions\View\ScheduledActions())->factoryTabView(WHMCS\Support\Ticket::find($id), $aInt->getAdminUser()->hasPermission("View Scheduled Ticket Actions"), $aInt->getAdminUser()->hasPermission("Create Scheduled Ticket Actions"))->render();
        WHMCS\Terminus::getInstance()->doExit();
    }
    if($action == "getclientlog") {
        ...
        if($action == "getquotedtext") {
            ...
            if($action == "mergetickets") {
                ...
                exit;
            } elseif($action == "blockdeletetickets") {
                check_token("WHMCS.admin.default");
                if(0 < count($selectedTickets)) {
                    foreach ($selectedTickets as $ticketId) {
                        $access = validateAdminTicketAccess($ticketId);
                ...
                exit;
            } elseif($action == "pintickets") {
                check_token("WHMCS.admin.default");
                if(count($selectedTickets) < 1) {
                    echo 0;
                    exit;
                }
                foreach ($selectedTickets as $ticketId) {
                    if(false !== validateAdminTicketAccess($ticketId)) {
                        echo 0;
                        exit;
                    }
                }
                ...
                foreach ($selectedTickets as $id) {
                    $ticket = WHMCS\Support\Ticket::find($id);
                    if(!is_null($ticket) && !$ticket->isPinned()) {
                        $ticket->pin($now)->save();
                        $ticket->logger()->withAction(AdminLang::trans("support.ticket.pin"))->withAdminAttribution(WHMCS\User\Admin::findOrFail(WHMCS\Auth::getID()))->log();
                    }
                }
                unset($ticket);
                unset($row);
                ...
                foreach ($selectedTickets as $id) {
                    ...
                    if(!is_null($ticket) && $ticket->isPinned()) {
                        $ticket->unpin()->save();
                        $ticket->logger()->withAction(AdminLang::trans("support.ticket.unpin"))->withAdminAttribution(WHMCS\User\Admin::findOrFail(WHMCS\Auth::getID()))->log();
                    }
                }
                unset($ticket);
                echo 1;
                ...
                if(!$action) {
                    ...
                    switch (validateAdminTicketAccess($mergeId[0])) {
                        ...
                        case false:
                        default:
                            WHMCS\Terminus::getInstance()->doDie(AdminLang::trans("support.mergeidnotfound"));
                            $masterTid = $id;
                ...
                } elseif($action == "viewticket" || $action == "view") {
                    $access = validateAdminTicketAccess($id);
                    if($access == "invalidid") {
                    ...
                    if($postreply || $postaction) {
                        ...
                        if($postaction == "note") {
                            ...
                            $serverRequest = WHMCS\Http\Message\ServerRequest::fromGlobals();
                            $serverRequest->attributes()->add(["ticketId" => $id]);
                            $actionResponse = (new WHMCS\Support\Ticket\ScheduledActions\TicketScheduledActionController())->createAction($serverRequest);
                        }
                        if($newstatus != $originalStatus) {
                    ...
                    if(!empty($deptid)) {
                        ...
                        $orig_flag = $data["flag"];
                        $orig_cc = $data["cc"];
                        $origPreventClientClosure = (bool) $data["prevent_client_closure"];
                        ...
                        if($orig_deptid != $deptid) {
                            ...
                            if($ticket->changeDept($deptid)) {
                                ...
                                $preventClientClosure = WHMCS\Support\Department::findOrFail($deptid)->preventClientClosure ?: $preventClientClosure;
                            }
                        }
                        ...
                        if($origPreventClientClosure !== $preventClientClosure && $orig_deptid == $deptid) {
                            addTicketLog($id, sprintf("Prevent client closure option %s", $preventClientClosure ? "enabled" : "disabled"));
                        }
                        $table = "tbltickets";
                        $array = ["status" => $status, "urgency" => $priority, "title" => $subject, "userid" => $userid, "cc" => $cc, "prevent_client_closure" => $preventClientClosure];
                        $where = ["id" => $id];
                ...
                if(!$action) {
                    ...
                    $selectors = implode(",", ["input[name='pin']", "input[name='unpin']", "input[name='merge']", "input[name='close']", "input[name='delete']", "input[name='blockdelete']"]);
                    $jqueryCode = "\$(\"" . $selectors . "\").on('click', function( event ) {\n    event.preventDefault();\n    var selectedItems = \$(\"input[name='selectedtickets[]']\");\n    var name = \$(this).attr('name');\n    switch(name) {\n        case 'merge':\n            var langConfirm = '" . $aInt->lang("support", "massmergeconfirm", "1") . "';\n            break;\n        case 'close':\n            var langConfirm = '" . $aInt->lang("support", "masscloseconfirm", "1") . "';\n            break;\n        case 'delete':\n            var langConfirm = '" . $aInt->lang("support", "massdeleteconfirm", "1") . "';\n            break;\n        case 'blockdelete':\n            var langConfirm = '" . $aInt->lang("support", "massblockdeleteconfirm", "1") . "';\n            break;\n        case 'pin':\n            var langConfirm = '" . $aInt->lang("support", "masspin", "1") . "';\n            break;\n        case 'unpin':\n            var langConfirm = '" . $aInt->lang("support", "massunpin", "1") . "';\n            break;\n    }\n    if (selectedItems.filter(':checked').length == 0) {\n        alert('" . $aInt->lang("global", "pleaseSelectForMassAction", "1") . "');\n    } else {\n        if (confirm(langConfirm)) {\n            ticketMassAction(name + 'tickets');\n        }\n    }\n});";
                    $actionButtonTpl = "<input type=\"submit\" value=\"%s\" name=\"pin\" class=\"btn btn-default btn-xs\" />\n<input type=\"submit\" value=\"%s\" name=\"unpin\" class=\"btn btn-default btn-xs\" />\n<input type=\"submit\" value=\"%s\" name=\"merge\" class=\"btn btn-default btn-xs\" />\n<input type=\"submit\" value=\"%s\" name=\"close\" class=\"btn btn-default btn-xs\" />";
                    $massactionbtns = sprintf($actionButtonTpl, AdminLang::trans("support.ticket.pin"), AdminLang::trans("support.ticket.unpin"), AdminLang::trans("clientsummary.merge"), AdminLang::trans("global.close"));
                    if($aInt->hasPermission("Delete Ticket")) {
                        $actionButtonTpl2 = "<input type=\"submit\" value=\"%s\" name=\"delete\" class=\"btn btn-danger btn-xs\" />\n<input type=\"submit\" value=\"%s\" name=\"blockdelete\" class=\"btn btn-danger btn-xs\" />";
                        $massactionbtns .= sprintf($actionButtonTpl2, AdminLang::trans("global.delete"), AdminLang::trans("support.blockanddelete"));
                    }
                    $name = "tickets";
                    $orderby = "lastreply";
                    $sort = "DESC";
                    ...
                    if(!$tag && !$searchFlag && (!$view || in_array($view, $activeAwaitingTicketStatuses))) {
                        ...
                        $ticketsModel->setShowPinnedFirst(true)->execute($criteria);
                        $ticketslist = $pageObj->getData();
                        $notFlaggedTo = WHMCS\Auth::getID();
                        $taggedTickets = $ticketsModel->getTagTicketIds();
                    }
                    $ticketFlagsMarkup = function ($ticket) {
                        $priorityIdentifier = $ticket->getPriorityIdentifier();
                        $priorityAlt = sprintf("%s: %s", AdminLang::trans("support.priority"), AdminLang::trans(sprintf("status.%s", $priorityIdentifier)));
                        if($ticket->isPinned()) {
                            $markup = sprintf("<i class=\"fas fa-thumbtack ticket-flag priority-%s\" aria-hidden=\"true\" title=\"%s\"></i>", $priorityIdentifier, sprintf("%s, %s", AdminLang::trans("support.ticket.pin"), $priorityAlt));
                        } else {
                            $markup = sprintf("<img src=\"images/%spriority.gif\" width=\"16\" height=\"16\" alt=\"%s\" title=\"%2\$s\" class=\"absmiddle\" />", $priorityIdentifier, $priorityAlt);
                        }
                        return $markup;
                    };
                    if(count($ticketslist)) {
                        foreach ($ticketslist as $ticket) {
                            ...
                            $tbl->addRow(["<input type=\"checkbox\" name=\"selectedtickets[]\" value=\"" . $ticket["id"] . "\" class=\"checkall\" />", ["tdAttributes" => ["class" => "text-nowrap text-center"], $ticketFlagsMarkup($ticket["ticketModel"])], $ticket["department"], "<a href=\"supporttickets.php?action=view&id=" . $ticket["id"] . "\"" . ($ticket["unread"] ? " style=\"font-weight:bold;\"" : "") . " title=\"" . $ticket["textsummary"] . "\">#" . $ticket["ticketnum"] . " - " . $ticket["subject"] . "</a>", $ticket["ticketModel"]->getRequestorDisplayLabel() . "<br>" . "<small>" . $ownerDisplayLabel . "</small>", $ticket["status"], $ticket["lastreply"]]);
                        }
                ...
                if($action == "viewticket" || $action == "view") {
                    ...
                    $preventClientClosure = (bool) $data["prevent_client_closure"];
                    $access = validateAdminTicketAccess($id);
                    if($access == "invalidid") {
                        $aInt->gracefulExit($aInt->lang("support", "ticketnotfound"));
                    }
                    ...
                    $smartyvalues["preventClientClosure"] = $preventClientClosure;
                    ...
                    $smartyvalues["numScheduledActions"] = number_format(WHMCS\Support\Ticket\ScheduledActions\TicketScheduledAction::getTicketUpcomingActions($id)->count());
                ...
                $aInt->templatevars = array_merge($aInt->templatevars, $smartyvalues);
                $aInt->display();
            }
        }
    }
