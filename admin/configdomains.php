...
if($action == "lookup-provider") {
    ...
    if(App::isInRequest("provider")) {
        ...
        } elseif(App::getFromRequest("provider") == "WhmcsDomains") {
...
if($action == "editpricing") {
    ...
    echo $infobox;
    echo "\n<p>\n    ";
    echo AdminLang::trans("domains.checkBoxToEnable");
    echo "    ";
    echo AdminLang::trans("domains.leaveAtNegativeOne");
    echo "    ";
    echo AdminLang::trans("domains.multiyearTransferNote");
    echo "</p>\n\n";
    ...
    if(!$noslabpricing) {
        ...
        while ($years <= 10) {
            ...
            foreach ($currenciesarray as $curr_id => $curr_code) {
                ...
                0 <= $registerValue || 0 <= $renewValue or $isEnabled = 0 <= $registerValue || 0 <= $renewValue || 0 <= $transferValue;
    ...
    if($action == "save") {
        ...
        if(!App::isInRequest("form_input_vars_check")) {
            redir("error=form_input_vars_exceeded");
        }
        $modifiedTlds = [];
    ...
    if($lookupProvider == "WhmcsDomains") {
        $lookupRegistrar = "<img src=\"" . $imgPath . "/lookup/whmcs-namespinning-large.png\">";
    } elseif($lookupProvider == "Registrar") {
    ...
    if($error) {
        ...
        if($error === "form_input_vars_exceeded") {
            $title = "global.validationerror";
            $error = AdminLang::trans("error.formInputVarsExceededError");
        }
    ...
    foreach ($registrarInterface->getActiveModules() as $activeRegistrar) {
        if($registrarInterface->load($activeRegistrar) && $registrarInterface->functionExists("GetTldPricing")) {
            ...
            echo AdminLang::trans("domains.duplicatetld");
            echo "        </a>\n    </p>\n\n</div>\n<input type=\"hidden\" name=\"form_input_vars_check\" value=\"1\" />\n</form>\n</div>\n    <div class=\"col-xs-3\"></div>\n    <div class=\"col-lg-3 col-xs-6\">\n        ";
            ...
            $errorGrowl = WHMCS\View\Helper::jsGrowlNotification("error", addslashes(AdminLang::trans("global.unexpectedError")), addslashes(AdminLang::trans("domains.enablePremiumDomainFailure")));
            $toggleDisabled = (int) $toggleDisabled;
            $errorSwal = ["title" => addslashes(AdminLang::trans("global.error")), "text" => addslashes(AdminLang::trans("domains.massUpdateError")), "confirmButtonText" => addslashes(AdminLang::trans("global.ok"))];
            $massUpdateSwal = ["title" => addslashes(AdminLang::trans("global.areYouSure")), "text" => addslashes(AdminLang::trans("domains.massUpdateConfirm")), "confirmButtonText" => addslashes(AdminLang::trans("global.yes")), "cancelButtonText" => addslashes(AdminLang::trans("global.no"))];
            $massUpdateUrl = routePath("admin-tld-mass-configuration");
            $massUpdateErrorGrowl = WHMCS\View\Helper::jsGrowlNotification("error", addslashes(AdminLang::trans("global.error")), addslashes(AdminLang::trans("global.unexpectedError")));
