    ...
    if(!$errormessage) {
        ...
        if(function_exists($module . "_SSLStepTwo")) {
            ...
            if($module == "marketconnect") {
                if(!$sslOrder->getDomain() && !empty($result["displaydata"]["Domain Name"])) {
                    $sslOrder->service->domain = $result["displaydata"]["Domain Name"];
            ...
            $domain = $sslOrder->getDomain();
            if(isset($result["displaydata"]["Organization"]) && 0 < strlen($result["displaydata"]["Organization"])) {
...
if($step == "3") {
    if($_SERVER["REQUEST_METHOD"] == "POST") {
        check_token();
    } elseif(WHMCS\Session::get("sslNoApproverEmails")) {
        ...
        foreach ($result["additionalfields"] as $heading => $fieldsconfig) {
            ...
            foreach ($fieldsconfig as $key => $configoption) {
                ...
                if($configoption["Type"] == "text") {
                    ...
                    $input .= "</select>";
                } elseif($configoption["Type"] == "country") {
...
if($step == "2") {
    if(count($result["approveremails"])) {
        if(isset($result["displaydata"]) && is_array($result["displaydata"])) {
            $additionalData = $result["displaydata"];
        }
