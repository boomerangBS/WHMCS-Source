...
class Form
{
    ...
    public function dropdown($name = [], $values = "", string $selected = "", string $onchange = false, $anyopt = false, $noneopt = 1, int $size = "", string $id = "form-control select-inline", string $cssClass = [], array $dataAttributes) : int
    {
        ...
        if(is_array($values)) {
            foreach ($values as $k => $v) {
                $value = $k;
                $selectedAttr = $k == $selected ? " selected=\"selected\"" : "";
                $color = "";
                $colorData = "";
                $valueDataAttributes = "";
                $text = "";
                if(is_array($v)) {
                    $color = $v["color"] ?? "";
                    if(!empty($v["dataAttributes"])) {
                        foreach ($v["dataAttributes"] as $dataName => $dataValue) {
                            $valueDataAttributes .= " data-" . $dataName . "=\"" . $dataValue . "\"";
                        }
                    }
                    if(!empty($v["text"])) {
                        $text = $v["text"];
                ...
                $colorAttr = $color ? " style=\"background-color:" . $color . "\"" : "";
                if(stristr($cssClass, "selectize")) {
                    $colorData = " data-data={\"colour\":\"" . $color . "\"}";
                }
                $attributes = sprintf("value=\"%s\"%s%s%s%s", $value, $selectedAttr, $colorAttr, $colorData, $valueDataAttributes);
                $code .= sprintf("<option %s>%s</option>", $attributes, $text);
            }
        } else {
            $code .= $values;
        }
