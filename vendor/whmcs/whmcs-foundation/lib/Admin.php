...
class Admin
{
    ...
    protected $popupwincount;
    protected $currentPageRequiresAuthConfirmation = false;
    private $allowNPSSurvey = true;
    ...
    public function populateStandardAdminSmartyVariables()
    {
        ...
        $this->assign("isCustomHeader", false);
        $minSidebar = false;
        ...
        foreach ($locales as $locale) {
            if($locale["language"] == \AdminLang::getName()) {
                ...
                $this->assign("adminsonline", $onlineAdmins);
                unset($onlineAdmins);
    ...
    public function output($jsonEncodingOptions = NULL)
    {
        if($this->isJsonResponseType()) {
            ...
            $footeroutput .= view("admin.utilities.date.footer");
            $footeroutput .= $this->retentlyNpsSurveys();
            ...
            if(Utility\MixPanel::isMixPanelTrackingEnabled()) {
                $mixPanelInitJs = Utility\MixPanel::getMixpanelInitJs($this->getAdminUser());
    ...
    {
        $bodypos = strpos($document, "</body>");
        if($bodypos === false) {
            $document .= $partial;
        } else {
            $document = substr($document, 0, $bodypos) . $partial . substr($document, $bodypos);
        }
        return $document;
    }
    ...
    public function setHelpLink($link) : \self
    {
        $this->helplink = $link;
        return $this;
    }
    ...
    public function beginAdminTabs(array $tabs = [], $defaultTabOpen = false, $tabPrefix = "")
    {
    ...
    public function clientSearchDropdown($name, $selectedOption = "", array $options = [], $placeHolder = "", $valueAttribute = "id", $tabOrder = 0)
    {
    ...
    {
        $this->currentPageRequiresAuthConfirmation = true;
    ...
    public function allowNPSSurvey($v)
    {
        $previous = $this->allowNPSSurvey;
        $this->allowNPSSurvey = $v;
        return $previous;
    }
    protected function retentlyNpsSurveys()
    {
        if(!$this->allowNPSSurvey) {
            return NULL;
        }
        if($this->currentPageRequiresAuthConfirmation && !$this->hasAuthConfirmation()) {
            return "";
        }
        $output = "";
        if(Admin\Survey\Retently\v1\GlobalNps::shouldRender($this->requiredPermission)) {
            Admin\Survey\Retently\v1\GlobalNps::ensureSettingVariant();
            $output .= "\n" . Admin\Survey\Retently\v1\GlobalNps::fromSetting()->generateOutput(User\Admin::getAuthenticatedUser());
        }
        if(Admin\Survey\Retently\v1\MarketConnectCsat::shouldRender($this->requiredPermission)) {
            Admin\Survey\Retently\v1\MarketConnectCsat::ensureSettingVariant();
            $output .= "\n" . Admin\Survey\Retently\v1\MarketConnectCsat::fromSetting()->generateOutput(User\Admin::getAuthenticatedUser());
        }
        return $output;
    }
}
