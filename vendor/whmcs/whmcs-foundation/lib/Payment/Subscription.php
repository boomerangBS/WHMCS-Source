...
class Subscription
{
    ...
    public static function cancel($relatedItem) : \WHMCS\Http\Message\JsonResponse
    {
        try {
            if($relatedItem instanceof \WHMCS\Service\Addon) {
                $logMessage = " - Service Addon ID: " . $relatedItem->id;
            } elseif($relatedItem instanceof \WHMCS\Domain\Domain) {
                $logMessage = " - Domain ID: " . $relatedItem->id;
            } elseif($relatedItem instanceof \WHMCS\Service\Service) {
            ...
            if(is_array($cancelResult) && ($cancelResult["status"] ?? "") == "success") {
                ...
                logTransaction($paymentMethod, $cancelResult["rawdata"] ?? "Unexpected module response", "Subscription Cancellation Success");
                ...
                logTransaction($paymentMethod, $cancelResult["rawdata"] ?? "Unexpected module response", "Subscription Cancellation Failed");
