...
class ComposerInstaller
{
    ...
    private function performEarlyFileCopy(array $files)
    {
        foreach ($files as $fileToCopy) {
            ...
            if($destSubDir) {
                ...
                if(!is_dir($destDir)) {
                    Utility\File::recursiveMkDir($this->targetDir, $destSubDir);
                }
            }
    ...
    public function relocateWhmcsCoreFiles($relocationMode)
    {
        ...
        $dirsWithCustomTargets = [];
        $excludeFromCopy = [];
        switch ($relocationMode) {
            case self::RELMODE_INSTALL:
            case self::RELMODE_UPDATE:
                $dirsWithCustomTargets = [UpdateConfig::DEFAULT_ADMIN_FOLDER => $this->updateConfig->getCustomAdminPath(), UpdateConfig::DEFAULT_CRON_FOLDER => $this->updateConfig->getCustomCronsDir()];
                $dirsToSkip = array_merge($dirsWithCustomTargets, [UpdateConfig::DEFAULT_ATTACHMENTS_FOLDER => $this->updateConfig->getCustomAttachmentsDir(), UpdateConfig::DEFAULT_COMPILED_TEMPLATES_FOLDER => $this->updateConfig->getCustomCompiledTemplatesDir(), UpdateConfig::DEFAULT_DOWNLOADS_FOLDER => $this->updateConfig->getCustomDownloadsDir()]);
                ...
                $this->output("Relocating WHMCS core files (dry-run)...");
                $issuePaths = Utility\File::recursiveCopyDryRun($this->sourceDir, $this->targetDir, $excludeFromCopy, false);
                if(!empty($issuePaths)) {
                    $this->output(\WHMCS\View\Markup\Error\Message\MatchDecorator\FilePermission\ApplyUpdateDryRun::getPathMessage($issuePaths));
                    try {
                        $this->output(sprintf("Cleaning up temporary install directory: %s", $this->sourceDir));
                        Utility\File::recursiveDelete($this->sourceDir);
                    } catch (\Exception $e) {
                        $this->output(sprintf("Cleanup failed: %s", $this->sourceDir));
                    }
                    throw new \WHMCS\Exception(\WHMCS\View\Markup\Error\Message\MatchDecorator\FilePermission\ApplyUpdateDryRun::getErrorMessage($issuePaths));
                }
                $this->output("Relocating WHMCS core files...");
                $this->performEarlyFileCopy(["init.php", "vendor/whmcs/whmcs-foundation/lib/Application.php"]);
                Utility\File::recursiveCopy($this->sourceDir, $this->targetDir, $excludeFromCopy);
                if(self::RELMODE_UPDATE === $relocationMode) {
                    foreach ($dirsWithCustomTargets as $defaultDir => $customPath) {
                        if($customPath) {
                            ...
                            Utility\File::recursiveCopy($this->sourceDir . DIRECTORY_SEPARATOR . $defaultDir, $this->normalizeDirSettingPath($customPath));
                        }
                    }
                }
