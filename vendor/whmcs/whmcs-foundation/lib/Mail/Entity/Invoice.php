...
class Invoice extends \WHMCS\Mail\Emailer
{
    protected function getEntitySpecificMergeData($invoiceId)
    {
        ...
        } elseif($invoice->getData("status") == \WHMCS\Billing\Invoice::STATUS_UNPAID && 0 < $invoice->getData("balance")) {
        ...
        $email_merge_fields["invoice_url"] = sprintf("%sviewinvoice.php?id=%d", $sysurl, $data["id"]);
        $email_merge_fields["invoice_link"] = sprintf($invoiceLink, $email_merge_fields["invoice_url"]);
