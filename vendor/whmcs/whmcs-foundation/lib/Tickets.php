...
class Tickets extends TableModel
{
    ...
    public $tagticketids = [];
    protected $showPinnedFirst = false;
    ...
    public function getTickets($criteria = [])
    {
        ...
        $query .= sprintf(" ORDER BY %s%s %s", $this->showPinnedFirst ? "pinned_at DESC, " : "", $this->getPageObj()->getOrderBy(), $this->getPageObj()->getSortDirection());
    ...
    private function buildCriteria($criteria)
    {
        ...
        } elseif($status == "Awaiting Reply" || $status == "awaitingreply" || $status == "") {
        ...
        } elseif($status == "All Tickets" || $status == "all" || $status == "any") {
        } elseif($status == "All Active Tickets" || $status == "active") {
            $statusfilter = Database\Capsule::table("tblticketstatuses")->whereShowactive(1)->pluck("title")->all();
            $filters[] = "tbltickets.status IN (" . db_build_in_array($statusfilter) . ")";
        } elseif($status == "Flagged Tickets" || $status == "flagged") {
            $statusfilter = Database\Capsule::table("tblticketstatuses")->whereShowactive(1)->pluck("title")->all();
    ...
    public function setDept($newdeptid)
    {
        ...
        $newDeptPreventClientClosure = Support\Department::findOrFail($newdeptid)->preventClientClosure;
        $newTicketPreventClientClosure = $newDeptPreventClientClosure ?: (bool) $this->data["prevent_client_closure"];
        update_query("tbltickets", ["did" => $newdeptid, "prevent_client_closure" => $newTicketPreventClientClosure], ["id" => $this->getData("id")]);
    ...
    public function getColoredStatusLabel($ticketStatus = false, $isAdmin)
    {
        if(!array_key_exists($ticketStatus, $ticketColors)) {
            $ticketColors[$ticketStatus] = get_query_val("tblticketstatuses", "color", ["title" => $ticketStatus]);
        }
        $color = $ticketColors[$ticketStatus];
        $translatedStatus = $this->getTranslatedTicketStatus($ticketStatus, $isAdmin);
        $statusLabel = "";
        if($color) {
    ...
    public function getTranslatedTicketStatus($status = false, $isAdmin)
    {
        $translationKey = self::getStatusLangKey($status, $isAdmin);
        if($this->hasTranslatedTicketStatus($status, $isAdmin)) {
            if($isAdmin) {
                return \AdminLang::trans($translationKey);
            }
            return \Lang::trans($translationKey);
        }
        return $status;
    }
    public function hasTranslatedTicketStatus($status = false, $isAdmin)
    {
        $translationKey = self::getStatusLangKey($status, $isAdmin);
        if($isAdmin) {
            return \AdminLang::self()->getCatalogue()->has($translationKey);
        }
        return \Lang::self()->getCatalogue()->has($translationKey);
    }
    protected static function getStatusLangKey($status, $isAdmin)
    {
        $statusKey = preg_replace("/[\\W\\s\\_]/", "", strtolower($status));
        if($isAdmin) {
            return "supportticketsstatus." . $statusKey;
        }
        return "supportticketsstatus" . $statusKey;
    }
    ...
    public function setShowPinnedFirst($showFirst) : \self
    {
        $this->showPinnedFirst = $showFirst;
        return $this;
    }
}
