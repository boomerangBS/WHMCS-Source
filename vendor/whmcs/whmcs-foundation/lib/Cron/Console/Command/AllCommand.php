...
class AllCommand extends AbstractCronCommand
{
    ...
    protected function getSystemQueue()
    {
        if($this->getHelper("daily-cron")->isDailyCronInvocation()) {
            $tasks = [new \WHMCS\Cron\Task\DataNormalization((new \WHMCS\Cron\Task\DataNormalization())->getDefaultAttributes()), new \WHMCS\Cron\Task\LicenseNotice((new \WHMCS\Cron\Task\LicenseNotice())->getDefaultAttributes()), new \WHMCS\Cron\Task\SystemConfiguration((new \WHMCS\Cron\Task\SystemConfiguration())->getDefaultAttributes()), new \WHMCS\Cron\Task\DatabaseBackup((new \WHMCS\Cron\Task\DatabaseBackup())->getDefaultAttributes()), new \WHMCS\Cron\Task\RefreshAppsFeed((new \WHMCS\Cron\Task\RefreshAppsFeed())->getDefaultAttributes())];
        } else {
            $tasks = [];
        }
