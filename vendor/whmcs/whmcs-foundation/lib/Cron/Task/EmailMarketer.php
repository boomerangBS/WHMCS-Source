...
class EmailMarketer extends \WHMCS\Scheduling\Task\AbstractTask
{
    ...
    public function __invoke()
    {
        ...
        foreach ($emailRules as $emailRule) {
            ...
            $clientnumdays = intval($settings["clientnumdays"]);
            ...
            $prodnumdays = intval($settings["prodnumdays"]);
            ...
            if($type == "client") {
                ...
                if(count($products)) {
                    ...
                    if(0 < $prodnumdays) {
                        ...
                        } elseif(in_array($prodfiltertype, ["beforedue", "before_due"])) {
                            $criteria[] = "nextduedate='" . date("Y-m-d", mktime(0, 0, 0, date("m"), date("d") + $prodnumdays, date("Y"))) . "'";
                        } elseif(in_array($prodfiltertype, ["afterdue", "after_due"])) {
                            $criteria[] = "nextduedate='" . date("Y-m-d", mktime(0, 0, 0, date("m"), date("d") - $prodnumdays, date("Y"))) . "'";
                        }
                    }
                    if(count($prodstatus)) {
