...
class HomepageController
{
    ...
    public function index(\WHMCS\Http\Message\ServerRequest $request)
    {
        ...
        $templatevars["homepagePromotionCarousel"] = (new \WHMCS\Promotions\PromotionController())->renderPromotionCarousel();
        ...
        if($roleId == 1) {
            if(!\WHMCS\Config\Setting::getValue("DisableSetupWizard")) {
                $aInt->allowNPSSurvey(false);
                $aInt->addHeadJqueryCode("openSetupWizard();");
            } elseif($aInt->hasPermission("View What's New") && $aInt->shouldSeeFeatureHighlights()) {
                $aInt->allowNPSSurvey(false);
    ...
    public function mentions(\WHMCS\Http\Message\ServerRequest $request)
    {
        ...
        foreach ($adminUsers as $adminUser) {
            $gravatar = sprintf("<img src=\"https://www.gravatar.com/avatar/%s?s=24&d=mm\" width=\"24\" height=\"24\" alt=\"Gravatar Icon\"/>", $adminUser->gravatarHash);
